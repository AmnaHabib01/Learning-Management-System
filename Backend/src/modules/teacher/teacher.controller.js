...
//     return res.status(201).json(new ApiResponse(201, response, "Teacher registered successfully"));
// });
...
const getTeacherById = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, response, "Teacher fetched successfully"));
});
...
const updateTeacher = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, response, "Teacher updated successfully"));
});
...
const getTotalTeachers = asyncHandler(async (req, res) => {
  const totalTeachers = await Teacher.countDocuments(); // only number
  return res
    .status(200)
    .json(
      new ApiResponse(
        200,
        { total: totalTeachers },
        "Total number of teachers retrieved successfully"
      )
    );
});
